...
function array2int(aArray) {
  ...
  console.log(aArray.charCodeAt(0) + ", " + aArray.charCodeAt(1) + ", " + aArray.charCodeAt(2) + ", " + aArray.charCodeAt(3) + " -> " + val);
  return val;
}

function pokeBit(aArray, aBit) {
  console.log("poking bit " + aBit);
  let cell = aBit / 8;
  let offset = aBit % 8;
  let val = aArray[cell];
  val |= (1 << offset);
