...
let blushPanel = panel.Panel({
  ...
  onMessage: function(aMessage) {
    ...
    if (aMessage == "blush") {
      ...
      monitor.recordEvent(kEvents.ADD_BLUSHLIST);
      this.hide();
    } else if (aMessage == "forget") {
      ForgetAboutSite.removeDataFromDomain(domain);
      monitor.recordEvent(kEvents.FORGET_SITE);
    } else {
      this.hide();
    }
  },
