...
function raiseConsent(aWindow, aURI) {
  let consentpanel = panel.Panel({
    ...
    onShow: function() {
      let query = bpUtil.getSearchTermFromURI(aURI);
      let type = query ? "query" : "domain";
      let category = (query ? bpCategorizer.getCategoryForQuery(query) :
                              bpCategorizer.getCategoryForHost(aURI.host));
      let value = query ? query : bpUtil.getBaseDomainFromHost(aURI.host);
      let message = {
        type: type,
        category: bpCategorizer.getExternalCategoryName(category),
        value: value
      };
