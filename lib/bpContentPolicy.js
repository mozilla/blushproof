  ...
  shouldLoad: function(aContentType, aContentLocation, aRequestOrigin,
    ...
    try {
      if (/^https?$/.test(aContentLocation.scheme) &&
        ...
        let correction = bpCategorizer.getCorrectSpelling(query);
        console.log("Query:", query, "correction:", correction);
        // TODO(mmc): Spellcheck against porny dictionary
        if ((bpCategorizer.getCategoryForBlushlist(aContentLocation.host) ||
             correction) && !isHostWhitelisted(aContentLocation.host)) {
          let win = null;
          let node = null;
